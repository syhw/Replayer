            ...
            plSymbol& Opening, plSymbol& X, plSymbol& Time)
{
    ...
    while (getline(inputstream, input))
    {
        ...
        if (tmpOpening != "")
        {
            ...
            std::set<int> tmpSet;
            tmpSet.insert(0);
            for (multimap<int, Building>::iterator it 
                    = tmpBuildings.begin(); 
...
OpeningPredictor::OpeningPredictor(const char* learningFileName)
{
	if (learningFileName[30] == 'P')
	{
		openings = OpeningPredictor::protoss_openings();
	    nbBuildings = NB_PROTOSS_BUILDINGS;
	    buildings_name = protoss_buildings_name;
	}
	else if (learningFileName[30] == 'T')
	{
		openings = OpeningPredictor::terran_openings();
	    nbBuildings = NB_TERRAN_BUILDINGS;
	    buildings_name = terran_buildings_name;
	}
	else if (learningFileName[30] == 'Z')
	{
		openings = OpeningPredictor::zerg_openings();
	...
	BWAPI::Broodwar->printf(">>>> Learning from: %s", learningFileName);
...
std::vector<std::string> OpeningPredictor::terran_openings()
{
    ...
    terran_openings.push_back("Unknown");
#endif
	return terran_openings;
}

std::vector<std::string> OpeningPredictor::protoss_openings()
{
    ...
    protoss_openings.push_back("Unknown");
#endif
	return protoss_openings;
}

std::vector<std::string> OpeningPredictor::zerg_openings()
{
...
int main(int argc, const char *argv[])
{
    ...
    OpeningPredictor op = OpeningPredictor(argv[1]);
